...
try {
    ...
    const include = (process.argv[4] || '*.cpp').split(';').filter(s => s.length != 0)
    const exclude = (process.argv[5] || '').split(';').filter(s => s.length != 0)
    const clang_tidy = (process.argv[6] || 'clang-tidy')
